/* ============================================
   A.E.G.I.S. MARK V — HOLOGRAPHIC AI ENGINE
...
(function () {
    ...
    const config = {
        ...
        userName: localStorage.getItem('aegis_user_name') || 'sir',
    };
    ...
    const classSchedule = {
        'Monday': [
            { name: 'Professional Issues in Information Systems and Technology', time: '10:00 AM - 01:00 PM', room: 'RM: 311 Lecture' },
            { name: 'Data and Digital Communications (Data Communications)', time: '01:00 PM - 04:00 PM', room: 'RM: 505 Lecture' }
        ],
        'Tuesday': [
            { name: 'Event-Driven Programming', time: '07:00 AM - 10:00 AM', room: 'COMPUTER LABORATORY 605' },
            { name: 'Event-Driven Programming', time: '01:00 PM - 03:00 PM', room: 'RM: 401 Lecture' }
        ],
        'Wednesday': [
            { name: 'Advanced Database Systems', time: '09:00 AM - 11:00 AM', room: 'RM: 403 Lecture' },
            { name: 'Enterprise Architecture', time: '11:00 AM - 01:00 PM', room: 'RM: 401 Lecture' }
        ],
        'Thursday': [
            { name: 'Advanced Database Systems', time: '10:00 AM - 01:00 PM', room: 'COMPUTER LABORATORY 604' },
            { name: 'Application Development and Emerging Technologies', time: '01:00 PM - 03:00 PM', room: 'RM: 402 Lecture' }
        ],
        'Friday': [
            { name: 'Enterprise Architecture', time: '10:00 AM - 01:00 PM', room: 'COMPUTER LABORATORY 604' },
            { name: 'Advanced Systems Integration and Architecture', time: '01:00 PM - 03:00 PM', room: 'RM: 312 Lecture' }
        ],
        'Saturday': [
            { name: 'Application Development and Emerging Technologies', time: '07:00 AM - 10:00 AM', room: 'COMPUTER LABORATORY 602' },
            { name: 'Advanced Systems Integration and Architecture', time: '10:00 AM - 01:00 PM', room: 'COMPUTER LABORATORY 604' }
        ],
        'Sunday': []
    };
    
    function getTodayScheduleString() {
        const days = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
        const today = days[new Date().getDay()];
        const todayClasses = classSchedule[today] || [];

        if (todayClasses.length === 0) {
            return `Today is ${today}. You have no scheduled classes today.`;
        }

        const classList = todayClasses.map(c => `- ${c.name} at ${c.time} (${c.room})`).join('\n');
        return `Today is ${today}. Here is your class schedule for today:\n${classList}`;
    }
    ...
    function init() {
        ...
        setTimeout(() => {
            ...
            const scheduleSummary = getTodayScheduleString(); // Get current day schedule

            if (config.apiKey) {
                const welcome = `${timeGreeting}, ${userTitle}. AEGIS is online. ${scheduleSummary}`;
                addMessage('ai', welcome);
                speak(welcome);
                logActivity('Gemini 3.5 Flash Core engaged');
            } else {
                const welcome = `${timeGreeting}, ${userTitle}. Voice systems standing by. ${scheduleSummary}`;
                addMessage('ai', welcome);
                speak(welcome);
                ...
                if (config.apiKey && config.apiKey.trim().length > 10) {
                    try {
                        ...
                        if (executedAppResponse) {
                            ...
                            speak(responseText);
                            logActivity('AEGIS audio response synthesized');
                        }
                    } catch (err) {
    ...
    async function callGeminiAudioAPI(base64Audio, mimeType) {
        const userTitle = config.userName || 'sir';
        const scheduleInfo = getTodayScheduleString();
...
- Today's Class Schedule Context: ${scheduleInfo}
...
- Format code blocks cleanly with syntax highlighting when asked for code.`;
    ...
    function generateOfflineResponse(input) {
        ...
        if (/class|schedule|subject|timetable/i.test(lower)) {
        return `${getTodayScheduleString()}, ${userTitle}.`;
        }
    ...
    function setupEventListeners() {
        ...
        els.saveKeyBtn.addEventListener('click', () => {
            ...
            localStorage.setItem('aegis_gemini_model', 'gemini-3.5-flash');
